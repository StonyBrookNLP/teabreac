...
def evaluate_json(
    golden_answers: Dict[str, Any], predicted_answers: Dict[str, Any]
) -> Tuple[float, float]:
...
def evaluate_prediction_file(gold_path: str, pred_path: str, output_path: str):

    golden_answers = json.load(open(gold_path, encoding="utf-8"))
    predicted_answers = json.load(open(pred_path, encoding="utf-8"))
    global_em, global_f1 = evaluate_json(golden_answers, predicted_answers)
...
if __name__ == "__main__":
    ...
    evaluate_prediction_file(args.gold_path, args.pred_path, args.output_path)
